      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6">
          {services.map((service, index) => (
            <div
              key={service.title}
              className={`relative bg-surface p-5 lg:p-8 rounded-2xl flex items-start gap-4 lg:flex-col lg:gap-0 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer border border-transparent hover:border-primary/20 overflow-hidden ${
                index === 0 ? 'ring-1 ring-primary/30 shadow-md' : ''
            ...
            >
              ...
              {/* Desktop-only absolute badge */}
              {service.tag && (
                <div className="hidden lg:block absolute top-4 right-4 bg-primary text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
                  {service.tag}
                </div>
              )}

              {/* Icon — bubble-shaped */}
              <div className="w-12 h-12 lg:w-14 lg:h-14 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0 text-primary lg:mb-6 group-hover:bg-primary group-hover:text-white group-hover:scale-110 transition-all duration-300">
                <service.icon className="w-5 h-5 lg:w-7 lg:h-7" />
              </div>

              {/* Text — flex-1 on mobile so it fills remaining space */}
              <div className="flex-1 min-w-0">
                {/* Mobile: title + inline badge in same row */}
                <div className="flex items-start justify-between gap-2 mb-1 lg:mb-3">
                  <h3 className="font-bold text-base lg:text-xl text-on-surface leading-snug">{service.title}</h3>
                  {service.tag && (
                    <span className="lg:hidden flex-shrink-0 bg-primary text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide whitespace-nowrap self-start mt-0.5">
                      {service.tag}
                    </span>
                  )}
                </div>
